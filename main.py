...
def run_3d_demo(out_dir: Path) -> None:
    ...
    plt.subplot(1, 2, 1)
    plt.imshow(image_iso, origin="lower", cmap="viridis", vmin=vmin, vmax=vmax)
    plt.title("Isotropic LOS")
    plt.subplot(1, 2, 2)
    plt.imshow(image_aniso, origin="lower", cmap="viridis", vmin=vmin, vmax=vmax)
    ...
    plt.figure(figsize=(8, 4))
    plt.subplot(1, 2, 1)
    plt.imshow(err_iso, origin="lower", cmap="coolwarm")
    plt.title("Isotropic Error")
    plt.subplot(1, 2, 2)
    plt.imshow(err_aniso, origin="lower", cmap="coolwarm")
    plt.title("Anisotropic Error")
    plt.tight_layout()
    plt.savefig(out_dir / "diffusion_3d_los_error.png", dpi=150)

    plt.figure()
    plt.imshow(analytic, origin="lower", cmap="viridis")
